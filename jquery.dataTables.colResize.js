 ...
 * @url         https://github.com/dhobi/datatables.colResize/blob/v1.7.2/jquery.dataTables.colResize.js
 * Language:    Javascript
 * License:     MIT
 */
(function (factory) {
  ...
  let ColResize = function (dt, opts) {
    ...
    let settings = new DataTable.Api(dt).settings()[0];
    dt = settings;
  ...
  $.extend(ColResize.prototype, {
          ...
          $columnNode.on(
            ...
            function (e) {
              ...
              if (self._fnIsInDragArea($node, e)) {
                ...
                self.s.state.originalTableWidth = $node
                  .closest("table")
                  .width();
    ...
    _fnGetAllColumns: function () {
      ...
      return columns.map((col) => {
        const { idx } = col;
        col.nTh = this.s.dt.api.columns(idx).header();
    ...
    _fnApplyWidth: function (changedWidth, element, column) {
      ...
      const $headerContainer = element.closest(".dt-scroll-headInner");
      let shouldChangeTableWidth =
        $headerContainer.length > 0 &&
        $table.width() + changedWidth > $headerContainer.width();
      ...
      let scrollBodyTh = element
        .closest(".dt-scroll")
        .find(
          ".dt-scroll-body table th:nth-child(" + (element.index() + 1) + ")"
        );
      ...
      let scrollFooterTh = element
        .closest(".dt-scroll")
        .find(
          ".dt-scroll-foot table th:nth-child(" + (element.index() + 1) + ")"
        );
      ...
      if (element.closest(".dt-scroll").length > 0) {
        ...
        this._fnGetAllColumns().forEach(function (column) {
          ...
          let $sbTh = element
            .closest(".dt-scroll")
            .find(
              ".dt-scroll-body table th:nth-child(" + (currentIndex + 1) + ")"
            );
          ...
          let $sfTh = element
            .closest(".dt-scroll")
            .find(
              ".dt-scroll-foot table th:nth-child(" + (currentIndex + 1) + ")"
            );
  ...
  if (
    ...
    // DataTables creation - check if the ColResize option has been defined on the
    // table and if so, initialise
    $(document).on("init.dt", function (e, settings, json) {
      var init = settings.oInit.colResize;
      var defaults = DataTable.defaults.colResize;

      if ((init || defaults) && !settings._colResize) {
  ...
  // API augmentation
  DataTable.Api.register("colResize.enable()", function () {
    return this.iterator("table", function (ctx) {
      ctx._colResize.fnEnable();
    });
  });
  DataTable.Api.register("colResize.disable()", function () {
    return this.iterator("table", function (ctx) {
  ...
  DataTable.Api.register("colResize.save()", function () {
    return this.iterator("table", function (ctx) {
      ctx._colResize.fnSaveState();
    });
  });
  DataTable.Api.register("colResize.restore()", function () {
    return this.iterator("table", function (ctx) {
